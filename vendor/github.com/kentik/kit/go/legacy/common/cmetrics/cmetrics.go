		...
		default:
			...
			case "influx", "influx_debug", "influx_quiet":
				...
				if strings.HasPrefix(dest[1], "http") || strings.HasPrefix(dest[1], "tcp") || strings.HasPrefix(dest[1], "udp") {
					// allow extra options to be passed after URL as ^key=val
					// e.g., influx:http://localhost:8186/write^keep_zero=true
					args := map[string]string{}
					argstrs := strings.Split(dest[1], "^")
					url := argstrs[0]
					if len(argstrs) > 1 {
						for _, arg := range argstrs[1:] {
					...
					l.Infof(log_prefix, "Metrics: Connecting Influx to [%s]: %s. [HTTP|TCP|UDP]", dest[0], url)
